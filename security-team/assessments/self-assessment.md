...
![Volcano architecture](volcano-architecture.svg)
...
- **Volcano Scheduler:** The scheduler makes workload placement and resource allocation decisions.
...
- **Volcano Agent:** `vc-agent` is an optional node-side component used by Volcano node resource management features.
...
When `vc-agent` features are deployed, node-side resource management becomes part of the Volcano deployment. The agent runs on nodes and may interact with node-level resource controls.
